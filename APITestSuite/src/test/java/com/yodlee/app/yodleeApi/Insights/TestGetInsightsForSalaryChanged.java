...
public class TestGetInsightsForSalaryChanged extends Base {
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("SalaryChanged");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\SalaryChanged\\GetSalaryChanged.csv")
	public void testGetNotificationsForSalaryChanged(String testCaseId, String testCaseDescription, String insightName,
		...
		User userInfo = User.builder().build();
		userInfo.setUsername("SalChanged" + System.currentTimeMillis());
			...
			if (!isCobrandSubscribed.isEmpty()) {
				try {
					String patchCobSubscriptionBodyParam = insightsHelper.constructPatchCobrandSubscriptionRequest(
							...
							"2", "", "", isCobrandSubscribed, "", "", thresholdNameValueType);
			...
			if (!isUserSubscribed.isEmpty()) {
				...
				if (!patchAccountIds.isEmpty()) {
					...
					patchUserSubscriptionBodyParam = insightsHelper.constructPatchUserSubscriptionRequest(insightName,
							"", "", "", "", "", "", "", "", "", entityParamsCount, "", entityParameterName, "2",
	...
	public void prepareAutomationReport() throws Exception {
		...
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary, "GET  SalaryChanged Insight", testInfo);
