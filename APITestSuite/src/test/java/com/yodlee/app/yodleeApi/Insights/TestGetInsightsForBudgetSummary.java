...
public class TestGetInsightsForBudgetSummary extends Base {
	...
	protected Logger logger = LoggerFactory.getLogger(TestGetInsightsForBudgetSummary.class);
	...
	public void setUp() throws Exception {	
	    ...
	    userInfo.setUsername("BudgetSummary" + System.currentTimeMillis());
		//userInfo.setUsername("CameInUndBud1607348990711");
		...
		Response response1 = providerAccountUtils.addProviderAccountStrict(providerId, "fieldarray",
				"budgetsummary.site16441.1", "site16441.1", sessionObj);
	    providerAccountId = response1.jsonPath().getLong(JSONPaths.PROVIDER_ACC_ID);
	...
	public void initializations() throws Exception {		
		...
		automationTestResults.setInsights("BudgetSummary");
		automationTestResults.setBuildNo("450");
		automationTestResults.setCodeCoveragePercent("78");
				
	}
	
	@Test(enabled = true, dataProvider = "feeder")
	@Source("\\TestData\\CSVFiles\\Insights\\BudgetSummary\\GetBudgetSummaryInsight.csv")
	public void testGetNotificationsForBudgetSummary(String testCaseId,String testCaseDescription,String insightName,String triggerCheckEnabled,String triggerType,String accountGroupName,
			String entityParameterCount,String entityParameterName,String categoryIds,String categoryIdsBudgetAmount,String categoryTypes,String categoryTypesBudgetAmount,String numberOfInsights,
			String editSubscription,String isCobrandSubscribed,	String isUserSubscribed,String patchEntityIds,String expectedInsightsForPatchedIds,String expectedNumberOfPatchedInsights,String validateKeys,String expectedStatus,String tcEnabled) throws Exception {
		...
		for(int ind1=0;ind1<accountGroupEntityIdsMap.size();ind1++){
			String message= budgetHelper.createBudgetForCatTypeDataAndCatData(accountGroupEntityIdsMap.get(accountGroupArray[ind1]), categoryIds.split("\\s+")[ind1], categoryIdsBudgetAmount.split("\\s+")[ind1], categoryTypes, categoryTypesBudgetAmount,sessionObj);
			if(message.equals("createBudgetResponse")) {
				expectedResultValues.setTestNotificationStatus(false);
				failureReason.setFailureReason("Budget NOT Created");
			}
			dbHelper.changeCreatedBudgetDateToPreviousMonth(dbUtility.getDBConnection(),insightsGenerics.getMemId(sessionObj).toString(),accountGroupEntityIdsMap.get(accountGroupArray[ind1]));
		}		
		...
		expectedResultValues = insightsGenerics.getExpectedAttributesToBeValidated(testCaseId, insightName, triggerType, "OTHER", entityParameterName, "", Integer.parseInt(numberOfInsights), numberOfKeys, 
		...
		for(int j=0;j<accountGroupArray.length;j++) {
			insightsGenerics.deleteCreatedAccountGroup(Integer.parseInt(commonEntity.getEntityIdsMap().get(accountGroupArray[j]).toString()), sessionObj);
