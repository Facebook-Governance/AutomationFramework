...
public class TestGetInsightsForBillPaid extends Base {
	...
	String viewEntitySchema = "BillPaidViewEntitySchema.json";
	String accountEntitySchema = "BillPaidAccountEntitySchema.json";
	InsightsCommons insightsCommons = new InsightsCommons();

	@BeforeClass(alwaysRun = true)
	public void setUp() throws Exception {
		System.out.println("Initiated execution for BillPaid");
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("BillPaidV2");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetBillPaid.csv")
	public void testGetNotificationsForBillPaid(String testCaseId, String testCaseDescription, String insightName,
		...
		User userInfo = User.builder().build();
		userInfo.setUsername("BillPaidV2" + System.currentTimeMillis());
		...
		if (Boolean.parseBoolean(editSubscription)) {
			if (!isCobrandSubscribed.isEmpty()) {
				try {
					String patchCobSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
							...
							thresholdNameValueType, "", "", editSubscription, isCobrandSubscribed, "", entityIdsMap);
					httpParams = insightsHelper.getHttpParams(InsightsConstants.COB_SUB_REQUEST, null, null, jwtCobAuthToken,
			...
			if (!isUserSubscribed.isEmpty()) {
				...
				if (!patchAccountIds.isEmpty()) {
					...
					patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
							InsightsConstants.USER_SUBSCRIPTION_CONTEXT, insightName, entityParameterName,
							thresholdNameValueType, "", "", editSubscription, isUserSubscribed,
							itemAccountIds.toString(), entityIdsMap);
				} else {
					patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
							InsightsConstants.USER_SUBSCRIPTION_CONTEXT, insightName, entityParameterName,
							thresholdNameValueType, "", "", editSubscription, isUserSubscribed,
		...
		httpParams=insightsHelper.getHttpParams(InsightsConstants.USER_NOTIFICATION_REQUEST, null, insightName, jwtUserAuthToken,null,"entityId","",null);
	...
	public void prepareAutomationReport() throws Exception {
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary, "GET  BillPaid Insight", testInfo);
