...
public class TestGetInsightsForExpensesExceededBudget extends Base {
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setStoryId("B-62546");
	}

	@Test(enabled = true, dataProvider = "feeder")
	@Source("\\TestData\\CSVFiles\\Insights\\ExpensesExceededBudget\\GetExpExceededBudgetInsight.csv")
	public void testGetNotificationsForExpensesExceededBudget(String testCaseId, String testCaseDescription,
			...
			String expectedStatus, String tcEnabled) throws Exception {
		...
		if (!accountGroupName.isEmpty())
		{
			for (int ind = 0; ind < accountGroupArray.length; ind++) {
				accountGroupId = insightsHelper.createAccountGroup(accountGroupArray[ind], sessionObj, entityIdsMap,accountName);
		...
		if (!accountGroupName.isEmpty())
		{
			for (int j = 0; j < accountGroupArray.length; j++) {
				insightsGenerics.deleteCreatedAccountGroup(
						Integer.parseInt(commonEntity.getEntityIdsMap().get(accountGroupArray[j]).toString()), sessionObj);
	...
	public void addAccount(long providerId, String dagUserName, String dagUserPassword, String inputContainers) {
		try {
			...
			if (!dagUserName.isEmpty()) {
				...
				userInfo.setUsername("ExpeExceedBudgt" + System.currentTimeMillis());
	...
	public void prepareAutomationReport() throws Exception {
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary, "GET ExpensesExceededBudget Insight",
				testInfo);
