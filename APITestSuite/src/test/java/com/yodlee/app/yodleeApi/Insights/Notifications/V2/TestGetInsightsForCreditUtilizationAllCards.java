...
public class TestGetInsightsForCreditUtilizationAllCards extends Base {


	public static Long providerAccountId = null;

	protected Logger logger = LoggerFactory.getLogger(TestGetInsightsForCreditUtilizationAllCards.class);
	...
	String accountEntitySchema = "CreditUtilizationAllCardsAccountEntitySchema.json";
	private String cobrandUser;
	...
	public void setUp() throws Exception {
	    ...
	    userInfo.setUsername("CreditUtilizeV2" + System.currentTimeMillis());
		//userInfo.setUsername("cutil2"); //sense user and wellness
		...
		jwtUserAuthToken = jwtHelper.getJWTToken(loginUser,envUser);
		jwtUserAuthToken = "Bearer ".concat(jwtUserAuthToken);	
		userId = insightsGenerics.getMemId(sessionObj);
	}

	public void linkAccounts(String credentials) throws Exception {
		System.out.println("initiated execution");
		String dagUser = credentials.split(",")[0];
		String dagPwd = credentials.split(",")[1];
		long providerId = 16441;
		Response response = providerAccountUtils.addProviderAccountStrict(providerId, "fieldarray",
				dagUser, dagPwd, sessionObj);
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("CreditUtilizationAllCards");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetCreditUtilizationAllCardsInsight.csv")
	public void testGetNotificationsForCreditUtilization(String testCaseId,String testCaseDescription,String insightName,String triggerCheckEnabled,
			...
			,String isCobrandSubscribed,String isUserSubscribed,String credentials,String patchEntityIds,String expectedInsightsForPatchedIds,String expectedNumberOfPatchedInsights,String validateKeys,String expectedStatus,String tcEnabled) throws Exception {

		commontUtils.isTCEnabled(tcEnabled, testCaseId);	
		if(!credentials.isEmpty())
			linkAccounts(credentials);
	...
	public void prepareAutomationReport() throws Exception {
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary,"GET  Creditcard Utilization All Cards Insight",testInfo);
