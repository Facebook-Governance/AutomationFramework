...
public class TestPeerSpendingComparisonSummary extends Base {

	public static Long providerAccountId = null;
	protected Logger logger = LoggerFactory.getLogger(TestPeerSpendingComparisonSummary.class);
	...
	public void setUp() throws Exception {
		System.out.println("Initiated execution for Peer Spending Comparison Summary");
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("CASHFLOW");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetPeerSpendingComparisonSummaryInsight.csv")
	public void testPeerSpendingComparisonSummary(String testCaseId, String testCaseDescription,
			String insightName, String verifyTriggerCheck, String triggerCheckEnabled, String inputContainers, String triggerType, long providerId,
			...
			String isCobrandSubscribed, String isUserSubscribed, String schema, String duration, String thresholdNameValueType, String accountDtls, String accountStatus, String frequency,
			String accountName, String patchEntityIds, String expectedInsightsForPatchedIds,
			String expectedNumberOfPatchedInsights, String validateKeys, String expectedStatus, String tcEnabled)
		...
		if (Boolean.parseBoolean(editSubscription)) {
				...
				try {
					if (!isCobrandSubscribed.equals("NA")) {
						String patchCobSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
								InsightsConstants.COB_SUBSCRIPTION_CONTEXT, insightName, entityParameterName, thresholdNameValueType, "",
			...
			if (entityParameterName.equalsIgnoreCase("view")) {
				String patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
						InsightsConstants.USER_SUBSCRIPTION_CONTEXT, insightName, entityParameterName, thresholdNameValueType, "", duration,
			...
			if (!isUserSubscribed.isEmpty()) {
				String patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
						InsightsConstants.USER_SUBSCRIPTION_CONTEXT, insightName, entityParameterName, thresholdNameValueType, frequency,
						duration, editSubscription, isUserSubscribed, patchEntityIds, entityIdsMap);
				httpParams = insightsHelper.getHttpParams(InsightsConstants.USER_SUB_REQUEST, null, null,
						jwtUserAuthToken, null, null, null, null);
				httpParams.setBodyParams(patchUserSubscriptionBodyParam);
				Response patchUserSubscriptionResponse = null;
		...
		if (!accountStatus.isEmpty()) {
			...
			String[] entityItems = accountDtls.split("\\s+");
			for (int i = 0; i < entityItems.length; i++) {
				String itemAccountId = entityIdsMap.get(entityItems[i]);
				if (i != entityItems.length - 1) {
					entityIds.append(itemAccountId);
			...
			for (int i = 0; i < itemAccountIdArray.length; i++)
				insightsDBUtils.updateAccountStatus(itemAccountIdArray[i],accountStatus);
		}
	...
	public void addAccount(long providerId, String dagUserName, String dagUserPassword, String inputContainers) {
		try {
			...
			if (!dagUserName.isEmpty()) {
				...
				userInfo.setUsername("CFS" + System.currentTimeMillis());
