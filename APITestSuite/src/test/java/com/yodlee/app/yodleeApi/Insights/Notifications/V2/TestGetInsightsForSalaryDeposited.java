...
public class TestGetInsightsForSalaryDeposited extends Base {
	...
	public void addAccounts(String dagUserName,String dagPassWord,String isCobrandSubscribed) throws Exception {
		insightsGenerics = new InsightsGenerics();
		insightsDBUtils = new InsightsDBUtils();
		String cobrand = prop.readPropertiesFile().getProperty("envCobrand");
		cobrandUser =  prop.readPropertiesFile().getProperty("cobrandUser");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetSalaryDepositedInsight.csv")
	public void testGetNotificationsForSalaryDeposited(String testCaseId,String testCaseDescription,String insightName,String triggerCheckEnabled,
			...
			,String isCobrandSubscribed,String isUserSubscribed,String dagCredentials,String updateCredentials,String patchEntityIds,String expectedInsightsForPatchedIds,String expectedNumberOfPatchedInsights,
			String validateKeys,String accountEntitySchema ,String expectedStatus,String tcEnabled) throws Exception {
		commontUtils.isTCEnabled(tcEnabled, testCaseId);
		if(!dagCredentials.isEmpty())
			addAccounts(dagCredentials.split(",")[0],dagCredentials.split(",")[1],isCobrandSubscribed);
		cobrandUser=prop.readPropertiesFile().getProperty("cobrandUser");
		jwtCobAuthToken = "Bearer ".concat(jwtHelper.getJWTToken(cobrandUser,envCobrand));	
