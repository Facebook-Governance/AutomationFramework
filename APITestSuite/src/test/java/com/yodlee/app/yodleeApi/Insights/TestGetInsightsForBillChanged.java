...
public class TestGetInsightsForBillChanged extends Base {
	public static Long providerAccountId = null;
	protected Logger logger = LoggerFactory.getLogger(TestGetInsightsForBillChanged.class);
	...
	public void setUp() throws Exception {
	    ...
	    userInfo.setUsername("BillAmtChanged" + System.currentTimeMillis());
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("BillChanged");
		automationTestResults.setBuildNo("474");
		automationTestResults.setCodeCoveragePercent("78");
		automationTestResults.setStoryId("B-XXXX");
	}	

	@Test(enabled = true, dataProvider = "feeder")
	@Source("\\TestData\\CSVFiles\\Insights\\BillChanged\\GetBillChanged.csv")
	public void testGetNotificationsForBillChanged(String testCaseId,String testCaseDescription,String insightName,String triggerCheckEnabled,
		...
		if(expectedResultValues.isUserPatched() && expectedResultValues.getCobrandSubscribed()) {	
				...
				for(int i=0;i<entities.size() && entities.get(i).getId()!=null && entities.get(i).isSubscribed();i++) {					
					...
					commonEntity = validateInsights.verifyGeneratedInsightsForPatchedEntites(userInsightsResponse,expectedResultValues,commonEntity,Integer.parseInt(expectedNumberOfPatchedInsights),thresholdValue);
	...
	public void prepareAutomationReport() throws Exception {
		...
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary, "GET  BillChanged Insight", testInfo);
