...
public class TestGetInsightsForPropertyValueChange extends Base {
	...
	public void setUp() throws Exception {
		System.out.println("Initiated execution for Change in Liabilities Insight");
		...
		/*
		 * insightsDBUtils.disableCustomerSubscriptions(envCobrand, jwtCobAuthToken,
		 * null);
		 */ }
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("Property Value Change");
		automationTestResults.setBuildNo("4XX");
		automationTestResults.setCodeCoveragePercent("85");
		automationTestResults.setStoryId("B-60433");
	}

	@Test(enabled = true, dataProvider = "feeder")
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetChangeInPropertyValue.csv")
	public void testGetNotificationsForChangeInPropertyValueChange(String testCaseId, String testCaseDescription,
			String insightName, String triggerCheckEnabled, String inputContainers, String triggerType,
			String accountType, long providerId, String dagUserName, String dagPassword, String accountGroupName,
			String entityParameterCount, String entityParameterName, String thresholdNameValueType,
			String categoryIdsBudgetAmount, String categoryTypes, String categoryTypesBudgetAmount,
			String numberOfInsights, String editSubscription, String isCobrandSubscribed, String isUserSubscribed,
			String schema, String duration, String frequency, String firstDataPointAmount, int firstDPdaysInPast,
			String firstDateOfDataPoint, String secondDataPointAmount, int secondDPdaysInPast,
			String secondDateOfDataPoint, String validateKeys, String expectedStatus, String tcEnabled)
		...
		addAccount(providerId, accountType, inputContainers, firstDataPointAmount, firstDPdaysInPast,
				firstDateOfDataPoint, secondDataPointAmount, secondDPdaysInPast, secondDateOfDataPoint, frequency);
		...
		if (Boolean.parseBoolean(editSubscription)) {
			...
			if (entityParameterName.equalsIgnoreCase("view")) {
				String patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
						InsightsConstants.USER_SUBSCRIPTION_CONTEXT, insightName, "account", thresholdNameValueType, "",
						duration, editSubscription, "False", "", entityIdsMap);
			...
			if (!isUserSubscribed.isEmpty()) {
				String patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
						...
						thresholdNameValueType, frequency, duration, editSubscription, isUserSubscribed, "",
						entityIdsMap);
				httpParams = insightsHelper.getHttpParams(InsightsConstants.USER_SUB_REQUEST, null, null,
						jwtUserAuthToken, null, null, null, null);
				httpParams.setBodyParams(patchUserSubscriptionBodyParam);
		...
		expectedResultValues = insightsGenerics.getExpectedAttributesToBeValidatedV2(testCaseId, insightName,
				...
				getCobrandSubscriptionMap(entityParameterName, isCobrandSubscribed), "".isEmpty() ? false : true, 0,
		...
		if (commonEntity.getExpectedResultValues().getTestNotificationStatus()) {
			...
			for (int i = 0; i < 15; i++) {
				...
				if (i == 9 && !(numberOfInsights.equals("0"))) {
					...
					Assert.fail(testInfo.getReasonForFailure());
				}
			}
	...
	public void addAccount(long providerId, String accountType, String inputContainers, String firstDataPointAmount,
			int firstDPdaysInPast, String firstDateOfDataPoint, String secondDataPointAmount, int secondDPdaysInPast,
			String secondDateOfDataPoint, String frequency) {
		try {
			...
			if (!accountType.isEmpty()) {
				...
				if (frequency.toUpperCase().contains("MONTHLY")) {
					...
					InsightsDBUtils.monthlyReadAndUpdateDataPointTable(loginUser, firstDataPointAmount,
							firstDPdaysInPast, firstDateOfDataPoint);
				} else if (frequency.toUpperCase().contains("WEEKLY")) {
					InsightsDBUtils.weeklyReadAndUpdateDataPointTable(loginUser, firstDataPointAmount,
							firstDPdaysInPast, firstDateOfDataPoint);
				}

				if (frequency.toUpperCase().contains("MONTHLY")) {
					InsightsDBUtils.monthlyReadAndUpdateDataPointTable(loginUser, secondDataPointAmount,
							secondDPdaysInPast, secondDateOfDataPoint);
				} else if (frequency.toUpperCase().contains("WEEKLY")) {
					InsightsDBUtils.weeklyReadAndUpdateDataPointTable(loginUser, secondDataPointAmount,
							secondDPdaysInPast, secondDateOfDataPoint);
				}

				providerAccountId = providerId;

				providerAccountUtils.addProviderAccountStrict(providerId, "fieldarray", "BnS2.bank1", "bank1",
						sessionObj);
				Thread.sleep(10000);
