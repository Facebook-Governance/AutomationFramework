...
public class TestGetInsightsForMostFrequentPurchases extends Base {
	...
	public void setUp() throws Exception {
	    ...
	    userInfo.setUsername("MostFrequentPurchase" + System.currentTimeMillis());
	...
	public void initializations() throws Exception {		
		...
		automationTestResults.setInsights("MostFrequentPurchases");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\MostFrequentPurchases\\GetMostFrequentPurchasesInsight.csv")
	public void testGetNotificationsForMostFrequentPurchases(String testCaseId,String testCaseDescription,String insightName,String verifyTriggerCheck,
		...
		if (Boolean.parseBoolean(editSubscription)) {
			if (!isCobrandSubscribed.isEmpty()) {		
				try {					
					if(!isCobrandSubscribed.equals("NA")) {
						String patchCobSubscriptionBodyParam = insightsCommons.constructCobrandandUserPatchRequest(InsightsConstants.COB_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,thresholdNameValueType,frequency,duration,editSubscription,isCobrandSubscribed,"",entityIdsMap);
			...
			if (!isUserSubscribed.isEmpty()) {    
				String patchUserSubscriptionBodyParam = insightsCommons.constructCobrandandUserPatchRequest(InsightsConstants.USER_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,thresholdNameValueType,frequency,duration,editSubscription,isUserSubscribed,patchEntityIds,entityIdsMap);
	...
	public void prepareAutomationReport() throws Exception {
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary,"GET  MostFrequentPurchases Insight",testInfo);		
