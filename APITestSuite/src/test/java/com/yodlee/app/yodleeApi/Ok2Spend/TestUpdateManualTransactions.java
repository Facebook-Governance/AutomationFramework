...
*
*/
public class TestUpdateManualTransactions extends Base {
	...
	@Source("\\TestData\\CSVFiles\\Ok2Spend_EndPoint\\UpdateManualTransactions.csv")
	public void testUpdateTransactionsForManual(String testcaseId, String testCase, String amount, String currency,
		...
		CreateTransactionsRequest createTransactionRequest = transactionsHelper.createManualTxnObject(210.23, "USD",
				"Consumer description", "CREDIT", 11, "SYSTEM", "0,0,1", "memo", container, "1223", "DEPOSIT",
				sessionObj);
		...
		String bodyParamForUpdate = senseTransactionsHelper.createManualTxnObjectUpdate(container, categoryId,
		...
		Response response = manualTransactionUtils.updateManualTransaction(httpParams, sessionObj);
		commonUtils.assertStatusCode(response, httpStatusCode);
	}

	@Test(enabled = true, dataProvider = "feeder")
	@Source("\\TestData\\CSVFiles\\Ok2Spend_EndPoint\\UpdateSystemPredWithManualAPI.csv")
	public void testUpdateTransactionsForSystem(String testcaseId, String testCase, String amount, String currency,
			String consumer, String baseType, int categoryId, String categorySource, String date, String memo,
			String container, String checkNumber, String reconStatus, String sourceType, String transactionType,
			int httpStatusCode,String ResFile,String FilePath, String enabled) throws Exception {
		if ("false".equalsIgnoreCase(enabled)) {
			throw new SkipException("Skipping Test Case ==> " + testCase);
		}
		System.out.println("Test Case Id is " + testcaseId + " test case is " + testCase);

		String updateTransactionsRequest = senseTransactionsHelper.createManualTxnObjectUpdate(container, categoryId,
				categorySource, consumer, memo, amount, baseType, reconStatus, checkNumber, date, "SYSTEM_PREDICTED",
				transactionType, currency);
		
		Integer transactionId = transactionsHelper.getSystemTransactionId(sessionObj, container);

		HashMap<String, Object> pathParam = new HashMap<String, Object>();
		pathParam.put("predictedTransactionId", transactionId);
		HttpMethodParameters httpParams = HttpMethodParameters.builder().build();
		httpParams.setPathParams(pathParam);
		httpParams.setBodyParams(updateTransactionsRequest);
