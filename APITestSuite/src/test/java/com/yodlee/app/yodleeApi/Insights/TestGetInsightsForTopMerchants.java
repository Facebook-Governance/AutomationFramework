...
public class TestGetInsightsForTopMerchants extends Base {
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("TopMerchants");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\TopMerchants\\GetTopMerchantInsights.csv")
	public void testGetNotificationsForTopMerchants(String testCaseId, String testCaseDescription,
		...
		Response postInvokerResponse = null, userInsightsResponse = null;
		try {
		...
		if (notificationTestStatus && entityParameterName.equalsIgnoreCase("account")) {
		...
		else if (notificationTestStatus && entityParameterName.equalsIgnoreCase("view"))
		{
	...
	public void addAccount(long providerId, String dagUserName, String dagUserPassword, String inputContainers) {
		try {
			...
			if (!dagUserName.isEmpty()) {
				...
				userInfo.setUsername("TopMerchants" + System.currentTimeMillis());
	...
	public void prepareAutomationReport() throws Exception {
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary, "GET TopMerchants Insight",
				testInfo);
