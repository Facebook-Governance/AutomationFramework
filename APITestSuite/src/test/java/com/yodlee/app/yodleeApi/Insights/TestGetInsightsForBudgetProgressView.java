...
public class TestGetInsightsForBudgetProgressView extends Base {
	public static Long providerAccountId = null;

	protected Logger logger = LoggerFactory.getLogger(TestGetInsightsForBudgetProgressView.class);
	...
	public void setUp() throws Exception {
	    ...
	    userInfo.setUsername("BudgetProgress" + System.currentTimeMillis());
	   ...
	   // userHelper.getUserSession(userInfo.getUsername(),userInfo.getPassword(),sessionObj)	;	
		...
		"BudgetProg.site16441.1", "site16441.1", sessionObj);
	...
	public void initializations() throws Exception {		
		...
		automationTestResults.setInsights("BudgetProgress");
		automationTestResults.setBuildNo("300");
		automationTestResults.setCodeCoveragePercent("78");
		automationTestResults.setStoryId("B-62494");
	}

	@Test(enabled = true, dataProvider = "feeder")
	@Source("\\TestData\\CSVFiles\\Insights\\BudgetProgress\\GetBudgetProgressInsight.csv")
	public void testGetNotificationsForBudgetProgress(String testCaseId,String testCaseDescription,String insightName,String verifyTriggerCheck,
			...
			String viewRuleAmountRangeDetails,String viewRuleCategoryIdDetails,String frequency,String expectedStatus,String tcEnabled) throws ParseException, InterruptedException, IOException {
		...
		if (Boolean.parseBoolean(editSubscription)) {
			if (!isCobrandSubscribed.isEmpty()) {		
				try {					
					if(!isCobrandSubscribed.equals("NA")) {
						String patchCobSubscriptionBodyParam = insightsCommons.constructCobrandandUserPatchRequest(InsightsConstants.COB_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,"",frequency,"",editSubscription,isCobrandSubscribed,"",entityIdsMap);
			...
			if (!isUserSubscribed.isEmpty()) {    
				String patchUserSubscriptionBodyParam = insightsCommons.constructCobrandandUserPatchRequest(InsightsConstants.USER_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,"",frequency,"",editSubscription,isUserSubscribed,patchEntityIds,entityIdsMap);
	...
	public void prepareAutomationReport() throws Exception {
		TestInfo testInfo = new TestInfo();
		testInfo = insightsHelper.printSummaryOfTests(notificationsTestSummary,"GET  BudgetProgress Insight",testInfo);		
