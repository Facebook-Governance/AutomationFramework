...
public class TestUpdateUserSegment extends Base {
	...
	public void setUp() throws Exception {
		System.out.println("initiated execution for UPDATE User Segment API");
		...
		User userInfo = User.builder().build();
	    userInfo.setUsername("UpdateSegment" + System.currentTimeMillis());
	...
	@Source("\\TestData\\CSVFiles\\Insights\\UpdateUserSegment.csv")
	public void testUpdateUserSegment(String testCaseId,String testCaseDescription,String income,String incomeCurrency,String incomeRange,String city,
		...
		Gson gson = new Gson();
		CreateSegment segment = insightsCommons.constructCreateUserSegmentPayload("1111", "USD", "", "", "", "", "", "", "", "", "", "");
		...
		CreateSegment patchSegment = insightsCommons.constructCreateUserSegmentPayload(income, incomeCurrency, incomeRange, city, state, country, zipCode, age, networthValue, networthCurrency, homeOwneShip, lifeStage);
		httpParams=insightsHelper.getHttpParams(InsightsConstants.USER_SUB_REQUEST, null, null, jwtUserAuthToken,null,null,null,null);
		httpParams.setBodyParams(gson.toJson(patchSegment));
		...
		Response patchUserSegmentResponse = insightUtils.createUserSegments(httpParams, null);
		if(patchUserSegmentResponse.getStatusCode()!=expStatusCode) {
			Assert.fail();
		}
		...
		if(patchUserSegmentResponse.getStatusCode() == com.yodlee.yodleeApi.constants.HttpStatus.NO_CONTENT_204)
		    insightsCommons.verifyGetUserSegmentResponse(patchSegment, getUserSegmentResponse);
		else
		    insightsCommons.verifyUserSegmentResponse(testCaseId,patchUserSegmentResponse.getStatusCode(),patchUserSegmentResponse,negativeExpectedValuesMap);
