...
public class TestEventInsightsSmoke extends Base {
	
	public static Long providerAccountId = null;

	protected Logger logger = LoggerFactory.getLogger(TestEventInsightsSmoke.class);
	...
	String cobrand = null,user = null;
	String[] insights = {"NEW_SUBSCRIPTION","LOW_CONFIDENCE_PROJECTED_SERIES","NEW_BILL","NEW_SALARY"};
	List<String> insightsList = Arrays.asList(insights); 

	
	public void setUp() throws Exception {
		insightsGenerics = new InsightsGenerics();
		insightsDBUtils = new InsightsDBUtils();
		cobrand = prop.readPropertiesFile().getProperty("envCobrand");
		...
		user = prop.readPropertiesFile().getProperty("envUser");
		envUser = (configurationObj.getApiVersion().equals("1.1") ? 
				  user: (configurationObj.getApiVersion().equals("2") ? user.substring(0, user.length() - 2) : null));
		userInfo.setUsername("smokerefresh" + System.currentTimeMillis());
	...
	public void addAccount(String addCredentials) throws Exception {
		String dagUserName = addCredentials.split(",")[0];
		String dagPassWord = addCredentials.split(",")[1];				
		insightsDBUtils.deleteGeneratedInsights(Long.parseLong(userId.toString()),envCobrand);
		long providerId = 16441;
		providerAccountId = providerId;
		Response response = providerAccountUtils.addProviderAccountStrict(providerId, "fieldarray", dagUserName,
				dagPassWord, sessionObj);
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetFeedsEventInsights.csv")
	public void testGetFeedsForEventTypeInsights(String testCaseId,String testCaseDescription,String insightName,String triggerType,String entityParamsCount,
			String entityParameterName,String thresholdNameValueType,String noumberOfInsights,String editSubscription
			...
			String viewRuleAmountRangeDetails,String viewRuleCategoryIdDetails,String viewRuleMerchantTypeDetails,String addCredentials,String updateCredentials,String jsonAccountSchemaFile,String jsonViewSchemaFile,String expectedStatus,String tcEnabled) throws Exception {
		...
		String jsonAccountSchema = null,jsonViewSchema = null;		
		JsonArray keysArray = null;
		int numberOfKeys = 0;
		if(!addCredentials.isEmpty() && !insightsList.contains(insightName)) {
			addAccount(addCredentials);
			...
			keysArray = insightsGenerics.getInsightKeys(insightName);
		...
		if (Boolean.parseBoolean(editSubscription)) {
			if (!isCobrandSubscribed.isEmpty()) {		
				try {					
					if(!isCobrandSubscribed.equals("NA")) {
						...
						if(insightName.equals("BILL_CHANGED") || insightName.equals("SUBSCRIPTION_CHANGED")) {
							patchCobSubscriptionBodyParam = insightsCommons.constructPatchSubscriptionRequestForCustomerAndUser(InsightsConstants.COB_SUBSCRIPTION_CONTEXT,insightName,"","","","","","",entityParameterName,thresholdNameValueType,"","",isCobrandSubscribed,"",entityIdsMap,"");
						}else {
							patchCobSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(InsightsConstants.COB_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,thresholdNameValueType,"","",editSubscription,isCobrandSubscribed,"",entityIdsMap);
						}						
			...
			if (!isUserSubscribed.isEmpty()) {  
				...
				if(insightName.equals("BILL_CHANGED") || insightName.equals("SUBSCRIPTION_CHANGED")) {
					patchUserSubscriptionBodyParam = insightsCommons.constructPatchSubscriptionRequestForCustomerAndUser(InsightsConstants.USER_SUBSCRIPTION_CONTEXT,insightName,"","","","","","",entityParameterName,thresholdNameValueType,"","",isUserSubscribed,patchEntityIds,entityIdsMap,"");
				}else {
					patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(InsightsConstants.USER_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,thresholdNameValueType,"","",editSubscription,isUserSubscribed,patchEntityIds,entityIdsMap);
				}					
		...
		if(insightsList.contains(insightName)) {
			addAccount(addCredentials);
			String dataLevels = insightsGenerics.getDataLevels(entityParameterName, isCobrandSubscribed, isUserSubscribed);
			insightsGenerics.setViewsPropertiesDetails(viewRuleAccountNames, viewRuleTransactionDetails,viewRuleAmountRangeDetails,viewRuleCategoryIdDetails,"",viewRuleMerchantTypeDetails,commonEntity);
			keysArray = insightsGenerics.getInsightKeys(insightName);
		...
		if(!updateCredentials.isEmpty())
			updateAccount(updateCredentials);
		...
		/*if(expectedResultValues.isUserPatched() && expectedResultValues.getCobrandSubscribed()) {	
				...
				for(int i=0;i<entities.size() && entities.get(i).getId()!=null;i++) {					
					...
					commonEntity = validateInsights.verifyGeneratedInsightsForPatchedEntitesV2(userInsightsResponse,expectedResultValues,commonEntity,expNoOfInsights,thresholdValue,jsonAccountSchemaFile,jsonViewSchemaFile);
		...
		if(commonEntity.getExpectedResultValues().getTestNotificationStatus()) {
			...
			httpParams.setHttpMethod("GET");
			userInsightsResponse = insightsUtilsV2.getInsightsFeeds(httpParams, null);
			commonEntity = validateInsights.verifyGeneratedInsightsV2(userInsightsResponse,expectedResultValues,commonEntity,Boolean.parseBoolean(isUserSubscribed),jsonAccountSchemaFile,jsonViewSchemaFile);	
         }
		 insightsDBUtils.deleteGeneratedInsights(Long.parseLong(userId.toString()),envCobrand);
