...
public class TestGetInsightsForFinancialForecast extends Base {
	...
	String accountEntityMonthlySchema = "FinancialForecastMonthlyAccountEntitySchema.json";
	String accountEntityWeeklySchema = "FinancialForecastWeeklyAccountEntitySchema.json";

	public void setUp(String dagCredentials) throws Exception {
		String username = dagCredentials.split("\\,")[0];
		String password = dagCredentials.split("\\,")[1];
		...
		Response response = providerAccountUtils.addProviderAccountStrict(providerId, "fieldarray", username, password,
				sessionObj);
	...
	public void initializations() throws Exception {
		...
		automationTestResults.setInsights("IncomeThresholdV2");
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetFinancialForecast.csv")
	public void testGetNotificationsForFinancialForecast(String testCaseId, String testCaseDescription,
			String insightName, String verifyTriggerCheck, String triggerType, String viewsAndAccountInsightsCount,
			String entityParamsCount, String entityParameterName, String noumberOfInsights, String editSubscription,
			String isCobrandSubscribed, String isUserSubscribed, String validateKeys, String frequency,
			String expectedStatus, String tcEnabled, String dagCredentials) throws Exception {
		commontUtils.isTCEnabled(tcEnabled, testCaseId);
		setUp(dagCredentials);
		...
		String accountEntitySchema = frequency.toLowerCase().trim().equals("weekly")
				|| frequency.toLowerCase().trim().equals("daily") ? accountEntityWeeklySchema
						: accountEntityMonthlySchema;

		if (frequency.trim().equalsIgnoreCase("daily")) {
			List<Integer> billerItemAccountIds = insightsDBUtils.getBillerAccountId(loginUser);
		...
		if (Boolean.parseBoolean(editSubscription)) {
				...
				try {
					if (!isCobrandSubscribed.equals("NA")) {
						String patchCobSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
								...
								frequency, "", editSubscription, isCobrandSubscribed, "", entityIdsMap);
			...
			if (!isUserSubscribed.isEmpty()) {
				String patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(
						InsightsConstants.USER_SUBSCRIPTION_CONTEXT, insightName, entityParameterName, "", frequency,
		...
		expectedResultValues = insightsGenerics.getExpectedAttributesToBeValidatedV2(testCaseId, insightName,
				...
				getCobrandSubscriptionMap(entityParameterName, isCobrandSubscribed), false,
				Integer.parseInt(noumberOfInsights), failureReason, keysArray, entityIdsMap, validateKeys,
		...
		if (expectedResultValues.isUserPatched() && expectedResultValues.getCobrandSubscribed()) {
			...
			for (int i = 0; i < entities.size() && entities.get(i).getId() != null; i++) {
				...
				commonEntity = validateInsights.verifyGeneratedInsightsForPatchedEntitesV2(userInsightsResponse,
						expectedResultValues, commonEntity, expNoOfInsights, "", accountEntitySchema, "");
		...
		if (commonEntity.getExpectedResultValues().getTestNotificationStatus()) {
			...
			commonEntity = validateInsights.verifyGeneratedInsightsV2(userInsightsResponse, expectedResultValues,
					commonEntity, Boolean.parseBoolean(isUserSubscribed), accountEntitySchema, "");
		}
		insightsDBUtils.deleteGeneratedInsights(Long.parseLong(userId.toString()), envCobrand);
