...
public class TestGetInsightsForIncomeThreshold extends Base {
	public static Long providerAccountId = null;

	protected Logger logger = LoggerFactory.getLogger(TestGetInsightsForIncomeThreshold.class);
	...
	String viewEntitySchema = "IncomeThresholdViewEntitySchema.json";
	String accountEntitySchema = "";
	...
	public void setUp() throws Exception {
	   ...
	   // userHelper.getUserSession(userInfo.getUsername(),userInfo.getPassword(),sessionObj)	;	
		...
		"IncomeThreshold.bank3", "bank3", sessionObj);
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetIncomeThreshold.csv")
	public void testGetNotificationsForIncomeExpenseOverview(String testCaseId,String testCaseDescription,String insightName,String verifyTriggerCheck,
			String triggerType,String viewsAndAccountInsightsCount,String entityParamsCount,String entityParameterName,String thresholdNameValueType,
String noumberOfInsights,String editSubscription
			...
			String viewRuleAmountRangeDetails,String viewRuleCategoryIdDetails,String frequency,String duration,String expectedStatus,String tcEnabled) throws Exception {
		...
		if (Boolean.parseBoolean(editSubscription)) {
			if (!isCobrandSubscribed.isEmpty()) {		
				try {					
					if(!isCobrandSubscribed.equals("NA")) {
						String patchCobSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(InsightsConstants.COB_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,thresholdNameValueType,frequency,duration,editSubscription,isCobrandSubscribed,"",entityIdsMap);
			...
			if (!isUserSubscribed.isEmpty()) {    
				String patchUserSubscriptionBodyParam = insightsCommons.constructSubscriptionPatchRequest(InsightsConstants.USER_SUBSCRIPTION_CONTEXT,insightName,entityParameterName,thresholdNameValueType,frequency,duration,editSubscription,isUserSubscribed,patchEntityIds,entityIdsMap);
		...
		if(expectedResultValues.isUserPatched() && expectedResultValues.getCobrandSubscribed()) {	
				...
				for(int i=0;i<entities.size() && entities.get(i).getId()!=null;i++) {					
					...
					commonEntity = validateInsights.verifyGeneratedInsightsForPatchedEntitesV2(userInsightsResponse,expectedResultValues,commonEntity,expNoOfInsights,"",accountEntitySchema,viewEntitySchema);
