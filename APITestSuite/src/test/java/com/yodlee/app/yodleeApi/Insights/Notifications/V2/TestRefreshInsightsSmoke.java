...
public class TestRefreshInsightsSmoke extends Base {
	
	public static Long providerAccountId = null;

	protected Logger logger = LoggerFactory.getLogger(TestRefreshInsightsSmoke.class);
	...
	public void setUp() throws Exception {
		...
		System.out.println("Smoke Suite for REFRESH Type insights");	
		...
		"refreshtypeinsight.site16441.2", "site16441.2", sessionObj);
	...
	@Source("\\TestData\\CSVFiles\\Insights\\V2Feeds\\GetFeedsRefreshInsights.csv")
	public void testGetFeedsForRefreshTypeInsights(String testCaseId,String testCaseDescription,String insightName,String verifyTriggerCheck,
			...
			String viewRuleAmountRangeDetails,String viewRuleCategoryIdDetails,String method,String updateCredentials,String jsonSchemaFile,String expectedStatus,String tcEnabled) throws Exception {
		...
		if(!updateCredentials.isEmpty() && method.equalsIgnoreCase("add")) {
			addAccount(updateCredentials);
		}
		if(!validateKeys.isEmpty()) {
			if(validateKeys.equalsIgnoreCase("account"))
				jsonAccountSchema=jsonSchemaFile;
			if(validateKeys.equalsIgnoreCase("view"))
				jsonViewSchema=jsonSchemaFile;
		}
		if(!updateCredentials.isEmpty() && method.equalsIgnoreCase("update"))
		...
		if(testCaseId.equals("AT-143093")) {
		...
		if(expectedResultValues.isUserPatched() && expectedResultValues.getCobrandSubscribed()) {	
				...
				for(int i=0;i<entities.size() && entities.get(i).getId()!=null;i++) {					
					...
					commonEntity = validateInsights.verifyGeneratedInsightsForPatchedEntitesV2(userInsightsResponse,expectedResultValues,commonEntity,expNoOfInsights,thresholdValue,jsonAccountSchema,jsonViewSchema);
		...
		if(commonEntity.getExpectedResultValues().getTestNotificationStatus()) {
			...
			httpParams.setHttpMethod("GET");
			userInsightsResponse = insightsUtilsV2.getInsightsFeeds(httpParams, null);
			commonEntity = validateInsights.verifyGeneratedInsightsV2(userInsightsResponse,expectedResultValues,commonEntity,Boolean.parseBoolean(isUserSubscribed),jsonAccountSchema,jsonViewSchema);		
         }
		 insightsDBUtils.deleteGeneratedInsights(Long.parseLong(userId.toString()),envCobrand);
