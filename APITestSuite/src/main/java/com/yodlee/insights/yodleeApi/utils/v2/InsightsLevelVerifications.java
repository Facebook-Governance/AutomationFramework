...
public class InsightsLevelVerifications {
	...
	public boolean verifyLowBalanceWarningNotificationsResponse(Response userInsightsResponse, String insightName,
			...
			String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason, String patchAccountIds,String insightExpectedKeys) {
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				...
				for (int i = 0; i < 1; i++) {
					...
					if((numberOfKeys*Integer.parseInt(expectedNumberOfInsights))!=keysArray.size()) {
						failNotificationTest(insightName,"Number of Keys in the notification are not matching expected = "+ numberOfKeys + " but found actual = " + keysArray.size(),
								failureReason);
					}
					...
					if(!expectedKeys.toString().equals(actualKeys.toString())) {
						failNotificationTest(insightName,"Keys in the notification are not correct expected = "+ expectedKeys.toString() + " but found actual = " + actualKeys.toString(),
								failureReason);
	...
	public boolean verifyCreditLimitWarningNotificationsResponse(Response userInsightsResponse, String insightName,
			String triggerType, String dataLevels, String entityParameterName, String thresholdNameValueType,
			String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason, String patchAccountIds,String insightExpectedKeys,StringBuilder validations) {
		notificationInsightsStatus = true;
		if (Integer.parseInt(expectedNumberOfInsights) == 0) {
			String insightResponse = userInsightsResponse.asString();
			boolean insightsNotAvailable = (insightResponse.equals("{}")) ? true : false;
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				...
				for (int i = 0; i < 1; i++) {
					...
					if (patchAccountIds.isEmpty()) {
						for (int j = 0; j < Integer.parseInt(expectedNumberOfInsights); j++) {
							try {
								String actualInsightValue = valuesArray.get(creditPercentPos).toString()
										.substring(1,valuesArray.get(creditPercentPos).toString().length() - 1);
	...
	public boolean verifyAllInsightsResponse(Response userInsightsResponse, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason,JsonArray expKeysArray,
		...
		if (Integer.parseInt(expectedNumberOfInsights) == 0) {
			...
			if (!insightsNotAvailable) {
				failNotificationTest(insightName,
						"Expected   "+expectedNumberOfInsights +" Insights but Actual Inssights "+insightsArray.size(), failureReason);
			}	
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				failNotificationTest(insightName, "Expected insights are not generated , Expected = "+ Integer.parseInt(expectedNumberOfInsights)
				+ "  Actual = " + actualInsights,failureReason);
			} else {
				for (int i = 0; i < notificationArray.size(); i++) {
					...
					if(!verifyCreatedDateTimeStamp(insightName, insightObj.get("createdDate").getAsString())) {
						failNotificationTest(insightName, "InValid createdDate either format is not correct OR created date may be greater than current date", failureReason);
					}					
					JsonObject dataEntity = insightObj.getAsJsonObject("data");
					JsonObject levelObject = dataEntity.getAsJsonObject("level");
					if (entityParameterName.equals("account") && !insightName.equals("CreditUtilization") && !insightName.equals("LiabilityChange") && !insightName.equals("PropertyValueChange")&& !insightName.equals("NetworthChange") && !insightName.equals("AssetChange")) {
						if (!entityIdsMap.containsValue(levelObject.get("id").getAsString())) {
							failNotificationTest(insightName,"Account Id = " + levelObject.get("id").getAsString() + " Doesn't Exists",failureReason);
						}
					}
					if (entityParameterName.equals("view")) {
						if (!entityIdsMap.containsValue(levelObject.get("id").getAsString())) {
							failNotificationTest(insightName,"View Id = " + levelObject.get("id").getAsString() + " Doesn't Exists",failureReason);
						}
					}
					if (entityParameterName.equals("accountGroup")) {
						if (!entityIdsMap.containsValue(levelObject.get("id").getAsString())) {
							failNotificationTest(insightName,"Account Group Id = " + levelObject.get("id").getAsString() + " Doesn't Exists",failureReason);
						}
					}
					if (entityParameterName.equals("goal")) {
						/*if (!entityIdsMap.containsValue(levelObject.get("id").getAsString())) {
							failNotificationTest(insightName,"Goal Id = " + levelObject.get("id").getAsString() + " Doesn't Exists",failureReason);
						}*/
					}
					...
					StringBuilder actualKeys=new StringBuilder();
					for(int index1=0;index1<keysArray.size();index1++) {
						if(keysArray.get(index1).getAsString().contains("merchant") && !insightName.equals("BillNotPaid") && !insightName.equals("LargeDepositNotice") && !insightName.equals("SalaryDeposited")
					...
					for(int keys=0;keys<expKeysArray.size();keys++) {
						...
						if (entityKeyObject.keySet().toString().contains(validateKeys)) {
							String[] keyArray  = entityKeyObject.get(entityKeyObject.keySet().toString().substring(1, entityKeyObject.keySet().toString().length()-1)).getAsString().split(",");
							for(int keys1=1;keys1<keyArray.length;keys1++) {
								if(merchantPresent && keyArray[keys1-1].toString().equals("transaction[0].date")) {
									expectedKeys.append(keyArray[0]+".transaction[0].merchant.name");//Large Deposit Notice  extra key will be appended when merchant is present
					...
					if(expectedNoOfKeys!=keysArray.size()) {
						failNotificationTest(insightName,"Number of Keys in the notification are not matching expected = "+ expectedNoOfKeys + " but found actual = " + keysArray.size(),
								failureReason);
					}
					...
					if(insightName.equals("LowBalanceWarning")) {
						int availableBalanceInitialPos = 5, totalKeysValues = 7;
						String actualGenereatedInsightValue = valuesArray.get(availableBalanceInitialPos).toString().substring(1,valuesArray.get(availableBalanceInitialPos).
								toString().length() - 4);
						if((int) Double.parseDouble(actualGenereatedInsightValue)>=thresholdValue) {
							failNotificationTest(insightName, "Expected value must be less than "
									+ thresholdValue + "But found actual value " + Integer.parseInt(actualGenereatedInsightValue),
									failureReason);
						}
					}
	...
	public boolean verifyInsightsForSmokeTest(Response userInsightsResponse, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason,JsonArray expKeysArray,
			HashMap<String,String> entityIdsMap,String validateKeys) {
		notificationInsightsStatus = true;
		JsonArray insightsArray = null;
	...
	public boolean verifyFinancialFeesInsightsResponse(Response userInsightsResponse, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason,JsonArray expKeysArray,
		...
		if (Integer.parseInt(expectedNumberOfInsights) == 0) {
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				...
				for (int i = 0; i < notificationArray.size(); i++) {
					...
					StringBuilder actualKeys=new StringBuilder();
					for(int index1=0;index1<keysArray.size();index1++) {
						if(keysArray.get(index1).getAsString().contains("merchant") && (!insightName.equals("TopMerchants"))) {
							System.out.println("MERCHANT Present -->" +keysArray.get(index1).getAsString());
							merchantPresent = true;
					...
					if((insightName.equals("FinancialFees") || insightName.equals("SpendingbyCategory") || insightName.equals("TopMerchants")) && !duration.equals("THIS_MONTH") && !validateKeys.equalsIgnoreCase("view"))
					{
	...
	public boolean verifyAllRefeshInsightsResponseForSmoke(Response userInsightsResponse, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason,JsonArray expKeysArray,
			HashMap<String,String> entityIdsMap,String validateKeys) {
		notificationInsightsStatus = true;
		JsonArray insightsArray = null;
	...
	public boolean verifyInsightsResponseUpcomingBillSub(Response userInsightsResponse, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason,JsonArray expKeysArray,
			HashMap<String,String> entityIdsMap,String validateKeys) {
		notificationInsightsStatus = true;
		JsonArray insightsArray = null;
		...
		if (Integer.parseInt(expectedNumberOfInsights) == 0) {
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				...
				for (int i = 0; i < notificationArray.size(); i++) {
					...
					for(int index1=0;index1<keysArray.size();index1++) {
						if(keysArray.get(index1).getAsString().contains("merchant") 
								&& !(insightName.equals("BillNotPaid") || insightName.equals("UpcomingBills") || insightName.equals("UpcomingSubscription"))) {                                      
							System.out.println("MERCHANT Present -->" +keysArray.get(index1).getAsString());
							merchantPresent = true;
						}
						actualKeys.append(keysArray.get(index1).getAsString());
	...
	public void verifyResponse(JsonObject insightObj, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights,  FailureReason failureReason,HashMap<String,String> entityIdsMap) {
		notificationInsightsStatus = true;
		JsonArray insightsArray = null;
		int thresholdValue = 0;
		if (!insightObj.get("name").getAsString().equals(insightName))
	...
	public boolean verifyInsightsResponseForLargeDepositNotice(Response userInsightsResponse, String insightName,String triggerType, String dataLevels, String entityParameterName,
			String thresholdNameValueType,String expectedNumberOfInsights, int numberOfKeys, FailureReason failureReason,JsonArray expKeysArray,
		...
		if (Integer.parseInt(expectedNumberOfInsights) == 0) {
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				...
				for (int i = 0; i < notificationArray.size(); i++) {
					...
					verifyResponse(insightObj,insightName,triggerType, dataLevels,entityParameterName,thresholdNameValueType,expectedNumberOfInsights,failureReason, entityIdsMap);
					JsonObject dataEntity = insightObj.getAsJsonObject("data");
					JsonObject levelObject = dataEntity.getAsJsonObject("level");
					JsonObject modelObject = dataEntity.getAsJsonObject("model");
					JsonArray keysArray = modelObject.getAsJsonArray("keys");
					...
					StringBuilder actualKeys=new StringBuilder();
					for(int index1=0;index1<keysArray.size();index1++) {
						if(keysArray.get(index1).getAsString().contains("merchant") && !insightName.equals("BillNotPaid")  
								&& !insightName.equals("LargeDepositNotice")&& !insightName.equals("UpcomingBills") 
								&& !insightName.equals("ProjectedInsufficientBalance") && !insightName.equals("MostFrequentPurchases") && !insightName.equals("IncomeThreshold")) {
							System.out.println("MERCHANT Present -->" +keysArray.get(index1).getAsString());
							merchantPresent = true;
						}
						actualKeys.append(keysArray.get(index1).getAsString());
		...
		if (Integer.parseInt(expectedNumberOfInsights) == 0) {
			...
			if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
				...
				for (int i = 0; i < notificationArray.size(); i++) {
					...
					for (int index1 = 0; index1 < keysArray.size(); index1++) {
						if (keysArray.get(index1).getAsString().contains("merchant")
								&& !insightName.equals("BillNotPaid") && !insightName.equals("BillPaid")&& !insightName.equals("SalaryChanged") 
								&& !insightName.equals("SubscriptionPaid") && !insightName.equals("LargeDepositNotice")
	...
	public boolean verifyResponseForTriggerCheckTests(Response userInsightsResponse, String expectedNumberOfInsights,
		...
		if (Integer.parseInt(expectedNumberOfInsights) != actualInsights) {
			failNotificationTest(
					insightName, "Expected insights are not generated Trigger check Failed, Expected = "
							+ Integer.parseInt(expectedNumberOfInsights) + "  Actual = " + actualInsights,
							failureReason);
		}
		return notificationInsightsStatus;
