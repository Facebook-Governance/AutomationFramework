...
public class InsightsHelper {
	...
	public String constructInvokerPayLoad(HashMap<String, String> entityIdsMap, String memId, String insightName,String envUser,String container,ArrayList<Long> cacheItemsList) {
		...
		if (envUser.equals("senseUserV2"))
			invokerPayLoad.setCobrandId(10019488L);
		if (envUser.equals("wellnessUser"))
			invokerPayLoad.setCobrandId(10011373L);
		if (envUser.equals("wellnessUserV2"))
			invokerPayLoad.setCobrandId(10011372L);
	...
	public void  summaryOfTests(Map<String,HashMap<String, Boolean>> testSummaryMap) {

		Set<String> allInsightNames = testSummaryMap.keySet();
		Iterator<String> iterateAllTests = allInsightNames.iterator();
		while(iterateAllTests.hasNext()) {
			String insghtName = iterateAllTests.next();
			System.out.println();
			System.out.println("GET CustomersSubscriptions API Execution Summary  of "+insghtName + " Insight");
			System.out.println();
			System.out.println("******************************************************************************");
			HashMap<String,Boolean> individualInsightTestSummaryMap = testSummaryMap.get(insghtName);
			Set<String> allTestCaseDescription = individualInsightTestSummaryMap.keySet();
			Iterator<String> iterateTestcases = allTestCaseDescription.iterator();
			while(iterateTestcases.hasNext()) {
				String testDescription = iterateTestcases.next();
				String status = individualInsightTestSummaryMap.get(testDescription) == true ?"PASSED":"FAILED";
				
				if(individualInsightTestSummaryMap.get(testDescription))
					System.out.println(testDescription +" -> "+status);
				else
					System.err.println(testDescription +" -> "+status);
	...
	public HttpMethodParameters getHttpParams(String requestType, String loginUser, String insightName, String jwtToken,
		...
		DEVELOPER_URL = prop.readPropertiesFile().getProperty("DEVELOPER_URL");
		INVOKER_URL = prop.readPropertiesFile().getProperty("INVOKER_URL");
		TASK_BASE_URI = prop.readPropertiesFile().getProperty("TASK_BASE_URI");
		...
		if (requestType.equals("notificationsRequest")) {
			...
			if(apiVersion.equals("2"))
				notificationsQueryParam.put("insightName", insightName);
			if (entityKey != null)
