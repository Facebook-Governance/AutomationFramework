 ...
 * @author sbhat1
 */
public class FincheckV2_RetirementSavings_Danger_Test extends TestBase {
	private static final Logger logger = LoggerFactory.getLogger(FincheckV2_RetirementSavings_Danger_Test.class);
	...
	public void navigateToIndicatorPage() {
		...
		v2GetStartd.quickOnboarding(PropsUtil.getDataPropertyValue("finV2.birthYear.1956"),
	...
	@Test(description = "AT-96883:Verify the score details text present", priority = 2, dependsOnMethods = "navigateToIndicatorPage")
	public void verifyScoreDetailsText() {
		...
		Assert.assertEquals(ret.getScoreDetailsText().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator4.scoreDetailsText"), "*** Details text mismatch.");
	}

	@Test(description = "AT-96884:Verify the back link is present on the indicator page", priority = 3, dependsOnMethods = "navigateToIndicatorPage")
	...
	public void verifySugestionText() {
		...
		Assert.assertEquals(ret.getSugestionText().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator4.danger.sugestionText").trim(),
				"*** Sugestion text mismatch.");
	}

	@Test(description = "AT-96885:Verify the health bar is displayed in indicator page", priority = 5, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96886:Verify Annual Gross Income Sections", priority = 8, dependsOnMethods = "navigateToIndicatorPage")
	public void verifyAnualGrossIncome() {
		logger.info(">>>>>Verifying the score details header is displayed");
		Assert.assertEquals(ret.getAnnualGrossIncomeText().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator4.annualGrossIncomeText"),
				"*** Gross Income text mismatch.");
		Assert.assertEquals(ret.getAnnualGrossIncomeValue().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator4.danger.annualGrossIncomeValue"),
				"*** Gross Income value mismatch.");
	}

	@Test(description = "AT-96886:Verify Suggested Savings Sections", priority = 9, dependsOnMethods = "navigateToIndicatorPage")
	public void verifySuggestedSavings() {
		...
		Assert.assertEquals(ret.getSuggestedSavingsValue().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator4.danger.suggestedSavingsValue"),
				"*** Savings value mismatch.");
	}

	@Test(description = "AT-96886:Verify About score section", priority = 11, dependsOnMethods = "navigateToIndicatorPage")
	public void verifyAboutScoreSection() {
		...
		Assert.assertEquals(ret.getAboutTheScoreText().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator.aboutScoreText"), "*** About Score mismatch.");
	}

	@Test(description = "AT-96887:Verify the take action header present", priority = 12, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96888:Verify the action button is displayed", priority = 14, dependsOnMethods = "navigateToIndicatorPage")
