...
public class FincheckV2_EmergencySavings_Danger_Test extends TestBase {
	private static final Logger logger = LoggerFactory.getLogger(FincheckV2_EmergencySavings_Danger_Test.class);
	...
	public void doInit() throws Exception {
		...
		accountAddition.addAggregatedAccount1(PropsUtil.getDataPropertyValue("finV2.emergencySavings.Danger.DagSite"),
				PropsUtil.getDataPropertyValue("finV2.emergencySavings.Danger.DagPassword"));
	...
	@Test(description = "AT-96844:Verify the score details text present", priority = 2, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96854:Verify the back link is present on the indicator page", priority = 3, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96846:Verify the sugestion Text Present below the healthbar", priority = 4, dependsOnMethods = "navigateToIndicatorPage")
	public void verifySugestionText() {
		...
		Assert.assertEquals(emergency.getSugestionText().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator3.danger.sugestionText").trim(),
				"*** Sugestion text mismatch.");
	}

	@Test(description = "AT-96847:Verify the health bar is displayed in indicator page", priority = 5, dependsOnMethods = "navigateToIndicatorPage")
	public void verifyHealthBar() {
		logger.info(">>>>> Verifying healthbar");
		Assert.assertTrue(emergency.getHealthBar().isDisplayed(), "*** Healthbar not displayed.");

	}

	@Test(description = "AT-96848:Verify the score details section header", priority = 7, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96859:Verify average spending in score details section", priority = 9, dependsOnMethods = "navigateToIndicatorPage")
	public void verifyAverageSpending() {
		...
		Assert.assertEquals(emergency.getAverageMonthlySpendingValue().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator3.danger.averageSpendingValue"),
				"*** Average spending value mismatched.");

	}

	@Test(description = "AT-96850:Verify current Coverage in score details section", priority = 10, dependsOnMethods = "navigateToIndicatorPage")
	public void verifyCurrentCovergae() {
		logger.info("Verifying Average Income Text and Value");
		Assert.assertEquals(emergency.getCurrentCoverageText().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator3.currentCoverageText"), "*** Current Coverage text.");
		Assert.assertEquals(emergency.getCurrentCoverageValue().getText().trim(),
				PropsUtil.getDataPropertyValue("finV2.Indicator3.danger.currentCoverageValue"));

	}

	@Test(description = "AT-96851:Verify About score section", priority = 11, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96852:Verify the take action header present", priority = 12, dependsOnMethods = "navigateToIndicatorPage")
	...
	@Test(description = "AT-96853:Verify the action button is displayed", priority = 14, dependsOnMethods = "navigateToIndicatorPage")
