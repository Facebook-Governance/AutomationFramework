...
public class AddManual_Transaction__Test extends TestBase {
	...
	void verifyAddManualTransactIcon() throws InterruptedException {
		...
		//utils.switchToFrame(d);
		SeleniumUtil.waitForElement("BBT", 8000);
	...
	@Test(description = "AT-29576:TRA_01_20:verify check filed", priority = 21, dependsOnMethods = { "verifyAddManualTransactPopHeader" })
	public void verifyCheckfiledForOntime() {
		
		//Verify the Note ,Check Number Fields  displayed After click on the "SHOW MORE OPTIONS" link
	...
	@Test(description = "AT-29579:TRA_01_21:Verify Check Number should not reflect while creating recurring transactions", priority = 22, dependsOnMethods = { "verifyCheckfiledForOntime" })
	...
	@Test(description = "AT-29576:TRA_01_22:verify note field for recurence transaction ", priority = 23,dependsOnMethods = { "verifyCheckfiledForOtherFq" })
	...
	{
       //Verify the "Cancel" and "ADD" labels displayed at the end of the Add a Transaction pop up
	...
	@Test(description = "AT-53679:TRA_01_25:Verify the pop up get closed after click on the X icon and cancel button in pop up", priority = 26,dependsOnMethods = { "cancelButton" })
	...
	@Test(description = "AT-53680 :TRA_01_26:Verify the pop up should get closed after click on close icon in pop up ", priority = 27,dependsOnMethods = { "cancelButtonClick" })
	...
	public void verifyAmountMandatoryField() {
		//Verify the below mandatory fields when user selects transaction type "Withdrawal/Deposit" Currency, Amount, Description, Transaction Type, Account, Frequency, Schedule Date, Category
        //Verify the message "This field is required" if user is not entered the values for mandatory fields
	...
	public void verifyInvestmentAmountMandatoryField() {
       //Verify the below mandatory fields when user selects transaction type "Investment"Currency, Amount, Description, Transaction Type, Account, Date, Symbol Ticker, CUSIP, Buy/Sell, Price, Quantity, Holding Type, Lot Holding
	...
	@Test(description = "AT-29586:TRA_01_35:verify the investment Scheduled date manadatory field", priority = 36,dependsOnMethods={"verifyInvestmentAmountMandatoryField"})
	...
	public void VeriftCurrencydropDown() {
		//Verify the Drop Down displayed after click on the "V" icon in the respective fields in pop up window
     	SeleniumUtil.waitForElement("BBT", 2000);
		if (PropsUtil.getEnvPropertyValue("MOBILEORWEB").equalsIgnoreCase("MOBILE")
				&& PropsUtil.getEnvPropertyValue("APPORBROWSER").equalsIgnoreCase("APP"))
	...
	public void VeriftCurrencydropDownValues() {
		//Verify only the Currency Symbols displayed in the dropdown Example:USD,INR,AUD,EUR etc..
	...
	void descerrorValidation() {
		//Verify the error message if user enters < 3 characters in the description field "At least 3 characters are required"
	...
	@Test(description = "AT-29590:TRA_01_48:Verify the maximum 130 characters allowed in the description field", priority = 49,dependsOnMethods={"descerrorValidation"})
	...
	@Test(description = "AT-29591:TRA_01_49:Verify the Amount field allow Max: 4 after decimal", priority = 50,dependsOnMethods={"descerrorValidation"})
	void amountDecimalmaxValueValidation() {
		//Verify the Amount field allow 11,4 logic   Max: 11 before decimal Max: 4 after decimal
	...
	@Test(description = "AT-29591,AT-29701:TRA_01_50:Verify the Amount field allow 11,4 logic   Max: 11 before decimal", priority = 51,dependsOnMethods={"descerrorValidation"})
	void amountValuemaxValueValidation() {
	//	Verify the Error message "Only 11 digits are allowed before decimal" if user entered more than 11 digits in amount ,price,net cost and quantity field
	...
	//	if (add_manual_transaction.getAtributeVAlue(add_manual_transaction.tag().getAttribute("id")).length() != 40) {
		//	Assert.assertTrue(false);
		//}
	}
	...
	public void tagMax3CharValidation() {
		//Verify the error message if user enters < 3 characters in the tag field "At least 3 characters are required"
	...
	@Test(description = "AT-29595:TRA_01_54:Verify the error message if user enters duplicate tags Tag name already exists", priority = 55,dependsOnMethods = { "descerrorValidation" })
	public void tagAlreadyexistValidation() {
		//Verify the error message if user enters duplicate tags ""Tag name already exists"
	...
	@Test(description = "AT-29596:TRA_01_55:Verify if any special characters are entered in Tags field, then show error Message : Only numbers and alphabets are allowed", priority = 56,dependsOnMethods = { "descerrorValidation" })
	public void tagSpecCharValidation() {
		//Verify if any special characters are entered in Tags field, then show error Message : "Only numbers and alphabets are allowed"
	...
	@Test(description = "AT-29597:TRA_01_56: Verify the user can able to add the unlimited tags", priority = 57,dependsOnMethods = { "descerrorValidation" })
	...
	@Test(description = "AT-29598:TRA_01_57:Verify the 150 characters allowed in the Note Field", priority = 58,dependsOnMethods = { "descerrorValidation" })
...
*/
	...
	@Test(description = "AT-29603:TRA_01_62:Verify the Error message Only 9 characters are allowed displayed in the CUSIP field if user enters  more than 9 characters", priority = 63,dependsOnMethods = { "symbolErrorValidation" })
	public void CUSIPValueValidation() {
        //Verify the Error message "Only 9 characters are allowed "displayed in the CUSIP field if user enters  more than 9 characters
	...
	@Test(description = "AT-29701::TRA_01_63:Verify the Error message Only 11 digits are allowed before decimal", priority = 64,dependsOnMethods = { "symbolErrorValidation" })
	...
	@Test(description = "AT-29604,AT-29702:TRA_01_64:Verify the price field allow 11,4 logic   Max: 11 before decimal Max: 4 after decimal", priority = 65, dependsOnMethods = { "symbolErrorValidation" })
	public void priceDecimalMaxValueValidation() {
		//Verify the price field allow 11,4 logic   Max: 11 before decimal Max: 4 after decimal
        //Verify the error message "Only 4 digits are allowed after decimal" if user enters more than 4 digits after decimal in amount,price and quantity field
		...
		Assert.assertEquals(add_manual_transaction.priceErr().getText(), PropsUtil.getDataPropertyValue("AmountalphaNumericErr2"));
		

	}
	
	@Test(description = "AT-29703:TRA_01_65:Verify the Error message Only numbers, decimal and formatters are allowed ( 123 345.00 )", priority = 66, dependsOnMethods = { "symbolErrorValidation" })
	public void priceAlphaNumValueValidation() {
		
	//	Verify the Error message "Only numbers, decimal and formatters are allowed ( 123 345.00 )" if user enters the some special characters and other characters in amount,price,netcost and quantity field

		add_manual_transaction.price().clear();
		add_manual_transaction.price().sendKeys(PropsUtil.getDataPropertyValue("AmountalphaNumeric"));
		add_manual_transaction.price().sendKeys(Keys.TAB);
		SeleniumUtil.waitForPageToLoad(500);
		Assert.assertEquals(add_manual_transaction.priceErr().getText(), PropsUtil.getDataPropertyValue("AmountalphaNumericErr"));
	}
	...
	void quantityMaxValidation() {
		//Verify the quantity field allow 11,4 logic   Max: 11 before decimal Max: 4 after decimal

		add_manual_transaction.quantity().clear();
		add_manual_transaction.quantity().sendKeys(PropsUtil.getDataPropertyValue("price2"));
		add_manual_transaction.quantity().sendKeys(Keys.TAB);
		SeleniumUtil.waitForPageToLoad(500);
		Assert.assertEquals(add_manual_transaction.quantityErr().getText(), PropsUtil.getDataPropertyValue("AmountalphaNumericErr2"));
	}
	...
	"symbolErrorValidation" })
     void quantityAlphaNumValidation() {
		...
		add_manual_transaction.quantity().sendKeys(Keys.TAB);
		SeleniumUtil.waitForPageToLoad(500);
		Assert.assertEquals(add_manual_transaction.quantityErr().getText(), PropsUtil.getDataPropertyValue("AmountalphaNumericErr"));
	}
	...
	public void ScheduleValueValidation() {
       //Verify the error message displayed if scheduled date is not entered"Invalid date"
	...
	@Test(description = "AT-29608:TRA_01_70:verify the  end date weekly error message", priority = 71,dependsOnMethods={"ScheduleValueValidation"})
	public void endDateweeklyValidation() {
      //Validate the error message on calendar if )end date - scheduled date) < Frequency selected. Example: "End date should be greater than schedule date at least by a week" Example: "End date should be greater than schedule date at least by 30 days "
	...
	@Test(description = "AT-29608:TRA_01_71:verify the end date two weekly error message", priority = 72,dependsOnMethods={"ScheduleValueValidation"})
	...
	@Test(description = "AT-29608:TRA_01_72:verify the end date twice monthly error message", priority = 73,dependsOnMethods={"ScheduleValueValidation"})
	...
	@Test(description = "AT-29608:TRA_01_73:verify the end date monthly error message", priority = 74,dependsOnMethods={"ScheduleValueValidation"})
	...
	@Test(description = "AT-29608:TRA_01_74:verify the end date two months once error message", priority = 75,dependsOnMethods={"ScheduleValueValidation"})
	...
	@Test(description = "AT-29608:TRA_01_75:verify the end date three months once error message", priority = 76,dependsOnMethods={"ScheduleValueValidation"})
	...
	@Test(description = "AT-29608:TRA_01_76:verify the end date half yearly once error message", priority = 77,dependsOnMethods={"ScheduleValueValidation"})
	...
	@Test(description = "AT-29608:TRA_01_77:verify the end date  yearly once error message", priority = 78,dependsOnMethods={"ScheduleValueValidation"})
	...
	public void catValueValidation() {
		//Validate categories are grouped as HLC - MLC - Sub Category in categories dropdown
	...
	@Test(description = "AT-29615:TRA_01_79:Validate Search Category functionality and usability in Select Category page", priority = 80,dependsOnMethods={"catValueValidation"})
	...
	public void dabitedLabel() {
		//Verify the "Debited from" text displayed in accounts dropdown if the transaction type selected is"Withdrawal"
	...
	@Test(description = "AT-29686:TRA_01_81:Verify the Credited to text displayed in the Accounts dropdown if the transaction type is Deposit", priority = 82,dependsOnMethods={"dabitedLabel"})
	public void creditedLable() {
		//Verify the "Credited to" text displayed in the Accounts dropdown if the transaction type is "Deposit"
	...
	@Test(description = "AT-29687:TRA_01_82:Verify the label Debited from displayed for the Investment Type if the type is Buy", priority = 83,dependsOnMethods={"dabitedLabel"})
	public void buyLable() {
		//Verify the label "Debited from" displayed for the Investment Type if the type is "Buy"
	...
	@Test(description = "AT-29688:TRA_01_83:Verify the label Credited To displayed for the Investment Type if the type is Sell", priority = 84,dependsOnMethods={"buyLable"})
	public void sellLable() {
		//Verify the label "Credited To" displayed for the Investment Type if the type is "Sell"
	...
	@Test(description = "AT-29692:TRA_01_84:Verify the Account Name - Account Type - Masked Account number reflecting in the Accounts dropdown", priority =85 ,dependsOnMethods={"dabitedLabel"})
	public void accountnumberMask() {
		//Verify the Account Name - Account Type - Masked Account number reflecting in the Accounts dropdown
...
@Test(description = "AT-29628:TRA_01_88:Verify single instance transactions created if user selects frequency as one time", priority = 89)
	public void PostedProjectedOntime() {
		//Verify single instance transactions created if user selects frequency as one time
	...
	public void verifytheCheck() {
		...
		for (int i = 0; i < add_manual_transaction.getAllPostedAmount1().size(); i++) {
			if (add_manual_transaction.getAllPostedAmount1().get(i).getText()
				...
				SeleniumUtil.waitForPageToLoad(2000);
				Assert.assertEquals(add_manual_transaction.AggregatedCheck().getAttribute("value"),PropsUtil.getDataPropertyValue("check12"));
			}
		}
	...
	public void veriftSupportedAccount() throws ParseException {
		//Verify the Bills,Rewards,Real Estate accounts should not reflect in the Manual Transaction pop up account dropdown
		SeleniumUtil.click(add_manual_transaction.addManualLink());
		SeleniumUtil.waitForPageToLoad(2000);
		SeleniumUtil.click(add_manual_transaction.accountdropdown());
		SeleniumUtil.waitForPageToLoad(1000);
