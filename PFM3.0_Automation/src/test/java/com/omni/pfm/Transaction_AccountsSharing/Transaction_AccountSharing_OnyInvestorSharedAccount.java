...
public class Transaction_AccountSharing_OnyInvestorSharedAccount extends TestBase {
	public static String TransactionloginName;

	private static final Logger logger = LoggerFactory.getLogger(Transaction_AccountSharing_OnyInvestorSharedAccount.class);
	LoginPage login;
	...
	@Test(description = "AT-70786", priority = 3)
	public void verifyMarkAspaidForInvestorAggregatedAccount_InInvestorView() throws Exception
	{	
		...
		if(markASPaid.MarkAsPaid().size()==3)
		{
			actual=true;
		}
	...
	public void verifyAddManualTransactionHiddenWhenAdvisorOnlySharedAcccount_InAdvisorView() throws Exception
	{
	   SAMLlogin.login(d, advisorUsername,InvesterName1,"10003700");
	   PageParser.forceNavigate("Transaction", d);
		SeleniumUtil.waitForPageToLoad(5000);
		Assert.assertTrue(accountDrapDown.AccountSharingAddManual().getAttribute("class").contains("hide"),
				"Add manual transaction should be disabled when investor has only Shared Account");
	}
	
	@Test(description = "verify Addvisor is added sucessfully", priority = 5)
	public void verifyMarkAsPAid_AdvisorView() throws Exception
	{
		PageParser.forceNavigate("Transaction", d);
		SeleniumUtil.waitForPageToLoad(5000);
		if(accountDrapDown.allTransactionAccount().get(0).getText().equals(""))
	...
	@Test(description = "AT-62060,AT-62069", priority = 7)
	public void verifyOnlyAdvisorSharedAccountLabel_AdvisorView() throws Exception
	{
		SeleniumUtil.click(accountDrapDown.accountFilterLink());
		SeleniumUtil.waitForPageToLoad(1000);
		Assert.assertEquals(accountDrapDown.allAccountLable().get(0).getText().trim(), PropsUtil.getDataPropertyValue("TransactionInvesterSharedAccount"), "only Investor shared accounts label should displayed");
	}
	
	@Test(description = "AT-62060", priority = 8)
	public void verifyMyAccountAndAllAccountShouldNotDisplayed_AdvisorView() throws Exception
	{
		...
		Assert.assertTrue(myAccountSize,"my account and all accounts should not dispalyed when advisor has only shared account ");
	}
}
