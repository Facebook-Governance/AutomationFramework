...
public class AccountAdditionMobile extends TestBase {
	...
	public void linkAccount() throws InterruptedException {
		...
		WebElement addLink = SeleniumUtil.getVisibileWebElement(d, "linkAccount", "LinkAnAccount", null);
	...
	public boolean addAggregatedAccount(String dagUserNaem, String dagPassword) throws AWTException, InterruptedException {
		...
		SeleniumUtil.click(SeleniumUtil.getVisibileWebElement(d, "siteSearch", "LinkAnAccount", null));
		...
		for (int i = 0; i < sitename.size(); i++) {
			if (sitename.get(i).getText().equals(PropsUtil.getDataPropertyValue("DAGSite"))) {
				SeleniumUtil.click(sitename.get(i));
				break;
			}
		}
		SeleniumUtil.waitForPageToLoad(2500);
		...
		try {
			...
			boolean status=false;
	         ...
	         WebElement successMsg = wait.until(ExpectedConditions.visibilityOf(SeleniumUtil.getWebElement(d, "MobileSucessIcon", "LinkAnAccount", null)));
